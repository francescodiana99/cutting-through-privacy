...
class ResNet(torch.nn.Module):
    def __init__(
        ...
        if stem == "CIFAR":
            ...
            conv1 = nn.Conv2d(channels, self.inplanes, kernel_size=7, stride=2, padding=3, bias=self.use_bias)
            bn1 = nn.BatchNorm2d(self.inplanes)
            nonlin = nn.ReLU()
...
if __name__ == "__main__":
    model = ResNet(BasicBlock, [2, 2, 2, 2], 3, 10)
    print(model)
